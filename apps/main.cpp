...
color ray_color(const ray& r) {
    ...
    // std::cerr <<  unit_direction.y() << ' ' << std::endl;
...
int main() {
    ...
    // const int image_width = 16000;
    const int image_width = 400;
    const int image_height = static_cast<int>(image_width / aspect_ratio);

    // Camera

    auto viewport_height = 2.0; // could be metres. Just in real world units.
    ...
    for (int j = image_height-1; j >= 0; --j) {
        ...
        for (int i = 0; i < image_width; ++i) {
            ...
            // Below will be the position of the pixel in the viewport. The ray I guess as a 3D vec?
